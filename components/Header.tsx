...
const Header: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
            ...
            <nav className="flex items-center space-x-8">
              {navItems.map((item) => (
                <NavLink
                  ...
                  to={item.href}
                  className={({ isActive }) => `
                    text-[13px] font-medium tracking-widest transition-all
          ...
          <button 
            ...
            className="lg:hidden p-2 text-white mobile-touch-target"
          >
            <div className="w-6 h-4 flex flex-col justify-between items-end">
              <span className={`h-[1px] bg-current transition-all ${isMenuOpen ? 'w-6 rotate-45 translate-y-[7.5px]' : 'w-6'}`}></span>
              <span className={`h-[1px] bg-current transition-all ${isMenuOpen ? 'opacity-0' : 'w-4'}`}></span>
              <span className={`h-[1px] bg-current transition-all ${isMenuOpen ? 'w-6 -rotate-45 -translate-y-[7.5px]' : 'w-6'}`}></span>
            </div>
          </button>
        </div>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden fixed inset-0 bg-[#0a1120] z-[90] transition-all duration-300 pt-32 px-10 ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
