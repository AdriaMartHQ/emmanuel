...
const Regions: React.FC = () => {
  return (
    <section id="regions" className="py-20 sm:py-32 bg-slate-950 text-white relative overflow-hidden">
      {/* 移动端减弱或隐藏复杂装饰，避免干扰视觉 */}
      <div className="absolute inset-0 opacity-5 sm:opacity-10 pointer-events-none overflow-hidden max-sm:hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row gap-12 sm:gap-20 items-start">
          <div className="lg:w-1/3 sm:sticky sm:top-32">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold mb-6 sm:mb-8 font-header">全球交流网络</h2>
          ...
          <div className="lg:w-2/3 grid gap-6 sm:gap-8 sm:grid-cols-2">
            {REGIONS.map((reg, idx) => (
              <div key={idx} className="group p-6 sm:p-10 bg-slate-900/50 backdrop-blur-sm border border-slate-800 active:border-blue-500/50 transition-all rounded-2xl relative overflow-hidden">
                {/* 移动端淡化装饰 Logo */}
                ...
                <h3 className="text-lg sm:text-xl font-bold text-white mb-5 flex items-center gap-3">
                  <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-600/20 text-blue-400 flex items-center justify-center text-[10px] font-bold">
                    0{idx + 1}
                  </span>
                  {reg.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {reg.nodes.map((node, i) => (
                    <span key={i} className="px-2.5 py-1 bg-slate-800/80 text-slate-300 text-[10px] sm:text-[11px] rounded-full border border-slate-700 font-medium">
                      {node}
                    </span>
                  ))}
                </div>
